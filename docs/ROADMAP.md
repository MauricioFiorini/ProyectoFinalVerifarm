...
| 1.14 | **Reparar `package-lock.json` otra vez y fijar el procedimiento para agregar dependencias.** El commit `ca338f5` (tarea 2.08) volvió a dejar el lock inconsistente: eliminó los nodos `@emnapi/core` y `@emnapi/runtime`, **exactamente los dos que había reparado la 1.13**, y `npm ci` falla de nuevo con `EUSAGE`. No es un error de quien lo corrió ni una particularidad de su máquina: son dependencias transitivas de paquetes **opcionales de plataforma** (`@img/sharp-wasm32`, `@tailwindcss/oxide-wasm32-wasi`) que npm poda del lock dejando colgadas las referencias de sus padres, que sí quedan. **La 1.13 no alcanzó** porque cambió el comando de arranque pero no el de agregar dependencias, que es donde se rompe. Incluye: reparar el lock con `npm install --package-lock-only`, declarar **`tsx`** —que `prisma/seed.ts` ejecuta sin que esté en `package.json` ni en el lock—, documentar el procedimiento de alta de dependencias en `docs/CONVENCIONES.md` y escribir la decisión `0003`. Ver `docs/decisiones/0003-como-se-agrega-una-dependencia.md`. | `[x]` | M | 1.13 |
| 1.15 | **Documentar el driver adapter de Prisma 7 y restituir el comentario de `src/lib/db.ts`.** Las cuatro dependencias que entraron con la 2.08 —`pg`, `@prisma/adapter-pg`, `@types/pg` y `dotenv`— son obligatorias en Prisma 7 para instanciar el cliente contra PostgreSQL, pero no hay nada escrito: el stack está declarado cerrado en `CONVENCIONES.md`, así que sin la decisión alguien las va a querer sacar sin saber por qué están. Se escribe como decisión `0004`. Además, el commit `d373370` borró de `src/lib/db.ts` las nueve líneas que explicaban por qué el cliente vive en `globalThis` —el pool de conexiones y las recargas de Next—, fuera del alcance de la 3.01: se restituyen, adaptadas al adapter nuevo. **No se toca el comportamiento del cliente, solo su documentación.** | `[x]` | S | 1.08, 2.08 |
...
| 3.07 | Modal de alta: nombre de droga, unidad de medida (select), stock mínimo, RxCUI **opcional, cargado a mano**. Botones **"Guardar"** y **"Cancelar"**. Errores por campo. El "opcional" quedó confirmado por la decisión `0005`; el nombre va sin dosis (`0006`). | `[x]` | M | 3.06, 3.02 |
| 3.08 | Estados de la tabla: cargando, vacía ("Todavía no hay medicamentos cargados") y error con botón **"Reintentar"**. | `[x]` | M | 3.06 |
...
| 4.13 | Modal de ingreso: número de lote, fecha de ingreso, vencimiento, cantidad. Botones **"Registrar"** y **"Cancelar"**. Validar que el vencimiento sea futuro. | `[x]` | M | 4.12 |
...
| 5.10 | Route handlers de pacientes, medicación y consultas. | `[x]` | M | 5.09 |
...
| 5.17 | Precarga: al elegir un paciente, sus medicamentos vigentes vienen seleccionados y se pueden agregar otros. | `[x]` | M | 5.16 |
...
| 5.19 | Aviso obligatorio en toda pantalla clínica: el sistema asiste, no reemplaza el criterio profesional. | `[x]` | S | 5.18 |
...
| 6.05 | Revisión responsive: usable en la notebook con la que se hace la defensa. | `[x]` | M | 6.03 |
...
| 6.13 | **Falta el paciente que muestra el tercer estado del módulo clínico.** La pantalla de resultado distingue "se encontraron interacciones" de "se revisó y no hay ninguna" y de "sin datos en la fuente" (5.18, decisión `0012`), pero el seed solo deja mostrar dos: `PAC-101` y `PAC-102` disparan interacciones y `PAC-103` no tiene cobertura en la fuente. **`PAC-104` no sirve**: Paracetamol y Amoxicilina tampoco están cubiertos, así que da "sin datos", igual que el 103. Hace falta un paciente con **dos drogas que la fuente cubra y que no interactúen entre sí**, verificado contra la base sembrada y no supuesto. **Sin consulta registrada**: la demostración lo evalúa en vivo, igual que `PAC-103`, y una consulta más movería la tarjeta de "consultas del día" que el guion afirma en 2. **Incluye sumar el caso a `docs/GUION_DEMOSTRACION.md`**, que hoy no lo recorre: el estado que prueba que el sistema distingue haber revisado de no haber podido revisar es el único que la demostración no puede enseñar. | `[x]` | M | 6.11 |
---
