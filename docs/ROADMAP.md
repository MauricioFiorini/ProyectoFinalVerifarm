...
| 2.10 | **Migración: `rxcui` opcional y `Lote.fechaIngreso`.** Pasar `Medicamento.rxcui` a `String? @unique` y agregar `fechaIngreso DateTime` a `Lote`. **No se edita la migración inicial**, se agrega una nueva. **No se agrega `cantidadIngresada`**: la cantidad vive en los movimientos. Ver las decisiones `0005` y `0007`. Las dos van en la misma migración para que los tres apliquen una sola. | X
